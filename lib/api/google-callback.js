...
function errorPage(message) {
  ...
  const origin = appUrl();
  return `<!doctype html><meta name="viewport" content="width=device-width"><body style="font-family:system-ui;padding:40px;text-align:center"><h2>Google connection failed</h2><p>${safe}</p><script>window.opener?.postMessage({type:'wyform-google-error',message:${JSON.stringify(String(message))}},${JSON.stringify(origin)});</script></body>`;
}

function successPage(email) {
  const origin = appUrl();
  const safeEmail = String(email || '').replace(/[&<>"']/g, (c) => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }[c]));
  return `<!doctype html><meta name="viewport" content="width=device-width"><body style="font-family:system-ui;padding:40px;text-align:center"><h2>Google connected</h2><p>${safeEmail ? `Connected as ${safeEmail}.` : 'Google is connected.'}</p><p>You can close this window and return to WyForm.</p><script>window.opener?.postMessage({type:'wyform-google-connected'},${JSON.stringify(origin)});setTimeout(()=>window.close(),900)</script></body>`;
}
