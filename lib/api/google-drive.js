import {supabaseAdmin,getUser,json,method,originOf,hostFromUrl,cors,securityHeaders} from './_supabase.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(JSON.stringify(body).length>20000)return json(res,413,{error:'Request is too large.'});
    if(action==='init'){
      const formKey=String(body.formKey||'').trim(),submissionId=String(body.submissionId||'').trim(),name=String(body.name||'attachment').replace(/[\x00-\x1f\/\\]/g,'_').slice(0,180),type=String(body.type||'application/octet-stream').slice(0,160),size=Number(body.size||0);
