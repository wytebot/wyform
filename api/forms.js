...
const patchable=['name','email_subject','auto_reply_subject','auto_reply_body','success_message','success_redirect','sheet_id','sheet_range','webhook_url','webhook_secret','notification_email'];
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='POST'){
      ...
      const form={owner_id:user.uid,name,domain,form_key:key(),active:true,branding:true,gmail_enabled:true,auto_reply:false,webhook_enabled:false,capture_utm:true,duplicate_window_minutes:10,retention_days:30,attachment_mode:'google_drive',version:1,webhook_secret:crypto.randomBytes(24).toString('base64url'),email_subject:`New ${name} submission`,success_message:'Thanks! Your message has been received.',allowed_origins:[domain],created_at:new Date().toISOString(),updated_at:new Date().toISOString()};
